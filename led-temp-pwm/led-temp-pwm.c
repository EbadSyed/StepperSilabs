...
int update_position(float x)
{
	...
	if(x<=255 && x>=0)
	{
		scaled_steps = x/255.0;
		scaled_steps = scaled_steps * MAX_STEPS;
	...
	if(scaled_steps>current_position)
	{
		move_steps = scaled_steps - current_position;
		//printf("\n Opening Valve");
		state=1;
		cw(move_steps);
	...
	else if(scaled_steps<current_position)
	{
		move_steps = current_position - scaled_steps;
		//printf("\n Closing Valve");
		state=1;
		ccw(move_steps);
		current_position = current_position - move_steps;
